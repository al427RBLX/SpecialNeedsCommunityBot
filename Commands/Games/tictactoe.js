module.exports = {
    ...
    category: 'Games',
    usage: '',
    description: 'Command used to Play Tic-Tac-Toe.',
    ...
    async execute(msg, args, cmd, client , Discord) {
        ...
        .setTimestamp();
                let opponent = msg.mentions.members.first() || msg.guild.members.cache.get(args[0])
        if(!opponent) return msg.channel.send(embed)
        let fighters = [msg.member.id, opponent.id].sort(() => (Math.random() > .5) ? 1 : -1)
        let Args = {
            user: 0,
            a1: {
                style: "gray",
        ...
        let message = await msg.channel.send(`**TicTacToe** | <@!${Args.userid}>'s turn (⭕)`)
        tictactoe(message)
